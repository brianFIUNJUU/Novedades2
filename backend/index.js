...
const sslOptions = {
  key: fs.readFileSync(path.join(__dirname, 'ssl/10.0.10.164-key.pem')),
  cert: fs.readFileSync(path.join(__dirname, 'ssl/10.0.10.164.pem'))
};
...
const io = socketIo(server, {
  cors: {
  methods: ["GET", "POST", "PUT", "DELETE"],
  origin: "https://10.0.10.164:4200", // URL del frontend  cambiar la ip luego de todo 
    methods: ["GET", "POST"],
...
console.log(`🚀 Servidor HTTPS con Socket.IO corriendo en https://10.0.10.164:${app.get('port')}`);
});
