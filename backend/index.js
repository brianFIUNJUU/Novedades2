...
// Middlewares
app.use(cors({
  origin: "https://192.168.100.7:4200",
...
const sslOptions = {
  key: fs.readFileSync(path.join(__dirname, 'ssl/192.168.100.7-key.pem')),
  cert: fs.readFileSync(path.join(__dirname, 'ssl/192.168.100.7.pem'))
};
...
const io = socketIo(server, {
  cors: {
  methods: ["GET", "POST", "PUT", "DELETE"],
  origin: "https://192.168.100.7:4200", // URL del frontend  cambiar la ip luego de todo 
    methods: ["GET", "POST"],
...
console.log(`🚀 Servidor HTTPS con Socket.IO corriendo en https://192.168.100.7:${app.get('port')}`);
});
